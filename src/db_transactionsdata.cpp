...
std::shared_ptr<QSqlQueryModel> CTransactionsData::GetSqlTableModelByCustomerName(QString const& strCustomerName)
{
	...
	lstDoubleColumnIndices.append(3);
	lstDoubleColumnIndices.append(4);
	lstDoubleColumnIndices.append(5);
	pCSqlQueryModel->SetDoubleColumnIndices(lstDoubleColumnIndices);

	std::shared_ptr<QSqlQueryModel> pTransactionSqlTableModel(pCSqlQueryModel);
	pTransactionSqlTableModel->setQuery(QString("SELECT deal.id, product.name, transaction_info.count, transaction_info.cost, deal.cost, deal.customer_cost, deal.date_time FROM "
		"transaction_info INNER JOIN deal INNER JOIN product "
		"ON deal.customer_id == %1 AND transaction_info.transaction_id == deal.id AND transaction_info.product_id == product.id").arg(strCustomerId));
